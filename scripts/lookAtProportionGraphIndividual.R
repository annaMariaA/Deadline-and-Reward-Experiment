...
subjectsToRemove = c(22,19,12,1,32)# 22 and 19 accuracy on homogenous trials below 50%, 12 RT on homogenous trials over 8s
...
aggData = (filter(fixdat, 
  ...
  fixNum>1, 
  targSideRel=="absent") 
  %>% group_by(subj,fixNum, incentive,block)
    %>% summarise(
...
plt = plt + labs(color="block")+scale_color_manual(labels = c("first", "second"),values = cbPalette )+ ggtitle("flat")
ggsave("plots/FixXsideByFlat.pdf", width=7, height=5)
ggsave("plots/FixXsideByFlat.jpg", width=7, height=5)
